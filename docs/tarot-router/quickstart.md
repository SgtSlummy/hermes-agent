...
Tarot Router release. It installs the signed Hermes `v1.0.9` GitHub release
and the signed Agents Council `v0.5.6` GitHub release without administrator
...
(
  ...
  expected="150e2b89c8f0e9365ee4e630f904b95a9501f87390e99d27b4aad46f17b89433"
...
--enable-free-mesh      (with --initialize-local; enroll all reviewed free routes and use existing protected credentials)
--skip-council
--verify-only
--model qwen2.5:3b
```
...
release `1.0.9` (the stable v1 metadata name), Hermes CLI package `0.14.0`,
contract `1.0.0`, Council state schema `3`, and `occult_initialized: false`.
A normal installation also records
Council `v0.5.6` and its archive hash; a skipped Council installation records
...
For the full unattended free-mesh pass, use the explicit free-mesh option:

```powershell
& $installer -InitializeLocal -EnableFreeMesh -Model "qwen2.5:3b"
```

```bash
sh "${TMPDIR:-/tmp}/install-occult.sh" --initialize-local --enable-free-mesh --model qwen2.5:3b
```

This scans every reviewed free-policy provider, activates keyless routes, and
uses only credentials already present in the protected Hermes environment.
Providers without existing authorization remain pending and are never silently
registered or contacted with fabricated credentials.
...
- Hermes Occult `v1.0.8`
- Agents Council `v0.5.5`

Download them from their GitHub release pages, verify their published checksum
files before extraction, disable Tarot Router, and retain the `v1.0.9` receipt
...
Move-Item -LiteralPath "$env:LOCALAPPDATA\Occult\occult-install-receipt.json" -Destination "$rollback\occult-install-receipt-v1.0.9.json"
```
...
mkdir -p "$rollback"
mv "${XDG_DATA_HOME:-$HOME/.local/share}/occult/occult-install-receipt.json" "$rollback/occult-install-receipt-v1.0.9.json"
```

Do not leave a receipt claiming `v1.0.9` in the active install root after the
rollback. The Hermes `v1.0.8` release asset contains its platform wheel; use
that wheel to replace the `uv` tool environment under the same install root.
Replace Council with the matching `v0.5.5` platform archive.

Runtime contract `1.0.0` and Council state schema `3` are unchanged in these
patch releases. Because the previous Hermes release predates the complete
starter deck, keep `occult.enabled` false while the previous binaries are
active; the previous gateway remains usable for non-Occult work. After
restoring `v1.0.9`, run `hermes tarot init` again to re-enable the full local
deck. State restoration is therefore not normally required, but the
